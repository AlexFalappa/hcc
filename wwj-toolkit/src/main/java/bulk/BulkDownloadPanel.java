 ...
 * @see BulkRetrievable
 */
public final class BulkDownloadPanel extends JPanel {
    ...
    protected JButton startButton2;
                ...
                BulkRetrievalThread thread = retrievable.makeLocal(this.currentSector, 0, this.cache,
                        new BulkRetrievalListener() {
                            ...
                            public void eventOccurred(BulkRetrievalEvent event) {
                                ...
                                System.out.printf("%s - %s%n", event.getItem(),
                                        event.getEventType().equals(BulkRetrievalEvent.RETRIEVAL_SUCCEEDED) ? "ok" : "FAILED/Unknown");
                            }
                        });
    ...
    @SuppressWarnings({"UnusedDeclaration"})
    protected void startButton2ActionPerformed(ActionEvent event) {
        double minLat, maxLat, minLon, maxLon;
        try {
            minLat = Double.parseDouble(txMinLat.getText());
            maxLat = Double.parseDouble(txMaxLat.getText());
            minLon = Double.parseDouble(txMinLon.getText());
            maxLon = Double.parseDouble(txMaxLon.getText());
            currentSector = Sector.fromDegrees(minLat, maxLat, minLon, maxLon);
            updateRetrievablePanels(currentSector);
            sectorLabel.setText(makeSectorDescription(currentSector));
            for (BulkRetrievablePanel panel : this.retrievables) {
                if (panel.selectCheckBox.isSelected()) {
                    BulkRetrievable retrievable = panel.retrievable;
                    BulkRetrievalThread thread = retrievable.makeLocal(this.currentSector, PlaceNameLayer.LEVEL_L, this.cache,
                            new BulkRetrievalListener() {
                                @Override
                                public void eventOccurred(BulkRetrievalEvent event) {
                                    // This is how you'd include a retrieval listener. Uncomment below to monitor downloads.
            ...
            JOptionPane.showMessageDialog(this, "Invalid numerical input");
        }
        this.getTopLevelAncestor().validate();
    }
    ...
    private void initComponents() {
        ...
        // numerical sector input
        JPanel sectPanel = new JPanel(new GridLayout(2, 5, 3, 3));
        sectPanel.setBorder(BorderFactory.createEmptyBorder(border, border, border, border));
        sectPanel.add(new JLabel("Min lat.", JLabel.RIGHT));
        txMinLat = new JTextField();
        sectPanel.add(txMinLat);
        sectPanel.add(new JLabel("Max lat.", JLabel.RIGHT));
        txMaxLat = new JTextField();
        sectPanel.add(txMaxLat);
        JButton bUpdNum = new JButton("Upd");
        bUpdNum.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                double minLat, maxLat, minLon, maxLon;
                try {
...
//                    System.out.println(tl.computeLevelForResolution(currentSector, 0.001));//0
//                    System.out.println(tl.computeLevelForResolution(currentSector, 0.0005));//1
//                    System.out.println(tl.computeLevelForResolution(currentSector, 0.0004));//2
//                    System.out.println(tl.computeLevelForResolution(currentSector, 0.0002));//3
//                    System.out.println(tl.computeLevelForResolution(currentSector, 0.0001));//4
                } catch (NumberFormatException numberFormatException) {
                    JOptionPane.showMessageDialog(BulkDownloadPanel.this, "Invalid numerical input");
                }
            }
        });
        sectPanel.add(bUpdNum);
        sectPanel.add(new JLabel("Min lon.", JLabel.RIGHT));
        txMinLon = new JTextField();
        sectPanel.add(txMinLon);
        sectPanel.add(new JLabel("Max lon.", JLabel.RIGHT));
        txMaxLon = new JTextField();
        sectPanel.add(txMaxLon);
        this.add(sectPanel);
        ...
        // Start button
        JPanel startPanel = new JPanel(new GridLayout(0, 2, 0, 0));
        startPanel.setBorder(BorderFactory.createEmptyBorder(border, border, border, border));
        startButton = new JButton("Start from selector");
        ...
        startButton2 = new JButton("Start numerical");
        startButton2.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent event) {
                startButton2ActionPerformed(event);
            }
        });
    ...
    private JTextField txMaxLon;
    private JTextField txMinLon;
    private JTextField txMaxLat;
    private JTextField txMinLat;
    ...
    public class DownloadMonitorPanel extends JPanel {
        ...
        private void initComponents() {
            ...
            if (text.length() > 20) {
                descriptionLabel.setToolTipText(text);
